...
fetch('./js/geojson/GeoJSON_regions.json').then(res => res.json()).then(countries => {
    ...
    const world = Globe({})
        ...
        .globeImageUrl('./imgs/earth-night.jpeg')
      ...
      <b>${d.regn_nm}</b> <br />
      Mortality: <i>${d.Mortality}</i> M$<br/>
      Lower: <i>${d.Lower}</i>Upper: <i>${d.Upper}</i>
    `)
        .onPolygonHover(hoverD => world
            .polygonAltitude(d => d === hoverD ? 0.12 : 0.04)
            .polygonCapColor('rgba(255, 255, 255, 0.2)')//d => d === hoverD ? colorGreys[1] : colorGreys[Math.ceil(Math.random() * 100)])
        )
    ...
    worldControls.autoRotateSpeed = .06;
